# ******************************************************
# Policy Comparison Case Study - Hypertension Treatment
# ******************************************************

#-# Note, this file is not meant to be executed on its own anymore. Use test_treatments.py
#-# to call the wrapper method and pass the desired treatment option as a string.

#-# ToDo: remove unused code, find which vars are being set as null and rm
#-# ToDo: Check for any code that can be optimized
#-# ToDo: Run multiple guideline code on a larger dataset (NHANES 40-60/75)
#-# ToDo: find a patient that will generate different results for each guidelines, preferably.
    #-# Right now all the results look the same, but that might just be because each guideline prescribes the same trt.
...
from JNC_8_guideline import jnc_8_guideline # 2014 JNC 8 guideline
from JNC_7_guideline import jnc_7_guideline # 2003 JNC 7 guideline
from esc_2018_guideline import esc_guideline # 2018 ESC/ESH guideline for arterial hypertension management
...
if os.name == 'posix': # name for linux system (for Dartmouth Babylons)
    ...
    #-# Changed directories to match my filesystem -Mateo Guideline Comparison\Output
    data_dir = os.path.abspath('G:\\.shortcut-targets-by-id\\1Tc06k-1aEZmEU8eFQzJLLaiJ7asR3-rf\\ASURE\\Data')
    home_dir = os.path.abspath('G:\\.shortcut-targets-by-id\\1Tc06k-1aEZmEU8eFQzJLLaiJ7asR3-rf\\ASURE\\Python\\Guideline Comparison\\Output')
    results_dir = os.path.abspath('G:\\.shortcut-targets-by-id\\1Tc06k-1aEZmEU8eFQzJLLaiJ7asR3-rf\\ASURE\\Python\\Guideline Comparison\\Output\\Results')
    fig_dir = os.path.abspath('G:\\.shortcut-targets-by-id\\1Tc06k-1aEZmEU8eFQzJLLaiJ7asR3-rf\\ASURE\\Python\\Guideline Comparison\\Output\\Figures')

def hypertension_treatment_policy_evaluation_function(treatment):
    ...
    for i in id_seq:
        ...
        print(tm.asctime(tm.localtime(tm.time()))[:-5], "Evaluating patient", i, "with policy", treatment)
        print("Estimated time remaining (for this policy) "+f"{estimatedTimeLeft_h:.0f}"+"h "+f"{estimatedTimeLeft_m:.0f}"+"m "+f"{estimatedTimeLeft_s:.0f}"+"s") #-# Print estimated time remining in "_h _m _s" format
        print("Estimated completion time (for this policy) "+tm.asctime(tm.localtime(tm.time()+estimatedTimeLeft))[:-5]) #-# Add estimated time remaining to current time to predict estimated time of completion in "Day Month Date H:M:S Year" format -Mateo
        ...
        if treatment == "aha":
            ...
            pi_trt = aha_guideline(ascvdrisk10, patientdata.sbp.values, patientdata.dbp.values,
                                   targetrisk, targetsbp, targetdbp, sbpmin, dbpmin,
                                   sbp_reduction, dbp_reduction, rel_risk_chd, rel_risk_stroke)
        elif treatment == "jnc8":
            #-# Calculating policies according to 2014 JNC 8 guidelines
            pi_trt = jnc_8_guideline(ascvdrisk10, patientdata.sbp.values, patientdata.dbp.values,
                                     targetrisk, sbpmin, dbpmin,
                                     sbp_reduction, dbp_reduction, rel_risk_chd, rel_risk_stroke, patientdata.age.values, patientdata.diab.values[0])
        elif treatment == "jnc7":
            #-# Calculating policies according to 2014 JNC 7 guidelines
            pi_trt = jnc_7_guideline(ascvdrisk10, patientdata.sbp.values, patientdata.dbp.values,
                                     targetrisk, sbpmin, dbpmin,
                                     sbp_reduction, dbp_reduction, rel_risk_chd, rel_risk_stroke, patientdata.diab.values[0])
        elif treatment == "esc":
            #-# Calculating policies according to ESC guidelines
            pi_trt = esc_guideline(ascvdrisk10, patientdata.sbp.values, patientdata.dbp.values,
                                   targetrisk, sbpmin, dbpmin,
                                   sbp_reduction, dbp_reduction, rel_risk_chd, rel_risk_stroke, patientdata.age.values)
        ...
        for t in reversed(range(years)): # number of decisions remaining
            ...
            if t == max(range(years)):
                ...
                Q_hat_next = Q_hat[t+1, pi_trt[t+1].astype(int)] # Q-value associated with next year's action
            ...
            # -# Commenting out all comparison code [start]
            ...
            # ci_width = [np.amax(d_alpha_reps[x]*np.sqrt((sigma2_hat_reps[x][feasible_t] + sigma2_hat_reps[x][pi_trt[t].astype(int)])/(x+1)))
            ...
            # Pi_epoch = np.where(Q_hat[t, pi_trt[t].astype(int)]-Q_hat[t, feasible_t] <=
            #                     d_alpha[t]*np.sqrt((sigma2_hat[t, feasible_t] +
            #                                                sigma2_hat[t, pi_trt[t].astype(int)])/reps))[0]
            #
        ...
        V_pi_trt = evaluate_pi(pi_trt.astype(int), tp, healthy, Lterm, disutility, discount)

        # Evaluating policy from AHA's guidelines in true transition probabilities in terms of ASCVD events
        trt_evt, trt_time_evt = evaluate_events(pi_trt.astype(int), tp, event_id)
        ...
        #         pi_best_range[t] = pi_trt[t].astype(int)
        #
        ...
        #     else: # Only the control action is part of the range
        #         pi_med_range[t] = pi_trt[t].astype(int)
        #
        ...
        #     else: # Only the control action is part of the range
        #         pi_fewest_range[t] = pi_trt[t].astype(int)
        #
        ...
        ptresults = pd.concat([pd.Series(np.repeat(i, years), name='id'),
                               ...
                               pd.Series(V_pi_trt, name='V_trt'), # patient's true value functions under AHA's guidelines
                               ...
                               pd.Series(pi_trt, name='pi_trt'), # patient's policy according to AHA's guidelines
                               ...
                               pd.Series(trt_evt, name='evt_trt'), # expected number of events under AHA's guideline
                               ...
                               pd.Series(trt_time_evt, name='time_evt_trt'), # expected years until an adverse event (including non-ASCVD related death) under AHA's guideline
        ...
        #-# Add name of treatment method to pt_sim column names
        ptresults = ptresults.rename(columns={"V_trt": "V_" + str(treatment), "pi_trt": "pi_" + str(treatment),
                                        "evt_trt": "evt_" + str(treatment),
                                        "time_evt_trt": "time_evt_" + str(treatment)})

        # Merging single patient data in data frame with data from of all patients
        pt_sim = pt_sim.append(ptresults, ignore_index=True) #-# append method is now deprecated, will have to be replaced by pandas.concat
        ...
        with open('Results for patients ' + str(min(id_seq)) + ' to ' + str(max(id_seq)) + ' until patient ' + str(i) + ' using treatment option ' +str(treatment)+ ' and ' + str(reps) +
                  ' batches.pkl', 'wb') as f: # full simulation
            pk.dump([#risk1, risk10, transitions,
                sb_Q, sb_sigma2, action_set, medication_set,
                pt_sim], f, protocol=3)
        if i >= 1: #-# Remove last generated file to save space
            os.remove('Results for patients ' + str(min(id_seq)) + ' to ' + str(max(id_seq)) + ' until patient ' + str(i-1) + ' using treatment option ' +str(treatment)+ ' and ' + str(reps) +
                      ' batches.pkl')
    return pt_sim
