...
raw_samples = [generate_sample(2, 0.5, N) for i in 1:10]
...
# we can look up any comparison and get the r_stats
r_stats_dict_K = Dict()
...
for i in 1:size
    for j in 1:size
        if i != j
            ...
            # make the comparison to get the root stats
            ### note the Z we're using is K
            r_stats = pairwise_comparison(s1, s2, K)
...
for i in 1:size
    for j in 1:size
        if i != j
            ...
            # make the comparison to get the root stats
            r_stats = pairwise_comparison(s1, s2, N)
            
            # compute the "outer se" of the comparison
            global_se = standard_error(sd1, sd2, M)
