...
function describe_experiment(sim)
    ...
    println("d: ", get_all_d(sim))
    println("Number of R stats: ", length(r_stats))
    println("Number of max R stats: ", length(r_max))
    println("17*8*B: ", 17 * 8 * B)
    println("(K(K-1))/2: ",  ( K*(K-1) /2 ) )

    r_hist = histogram(r_stats, title = "Distribution of Rs") 
    r_max_hist = histogram(r_max, title = "Distribution of max(R)s")
...
for i in 1:1000
    ...
    s1 = generate_sample(0.0, 0.25, n)
    s2 = generate_sample(0.0, 0.25, n)
