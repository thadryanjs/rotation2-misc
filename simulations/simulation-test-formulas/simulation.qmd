...
sample1 = get_batch_info(generate_sample(2, 0.5, N), "sample1", K)
sample2 = get_batch_info(generate_sample(2, 0.5, N), "sample2", K);
```

```{julia}

experiment = run_simulation([sample1, sample2], B)
```

```{julia}

experiment = computeCIs!(experiment, 0.05, N)
```
...
samples |> length == samples_per_experiment
```
...
r_stats_N = get_all_rstats(sim_N)

r_max_N = get_all_rmax(sim_N)

println("Number of R stats: ", length(r_stats_N))
println("Number of max R stats: ", length(r_max_N))
```
...
r_stats_K = get_all_rstats(sim_K)

max_rstats_K = get_all_rmax(sim_K) 
```
...
r_stats_B = get_all_rstats(sim_B)

max_rstats_B = get_all_rmax(sim_B)
```
...
# TODO
    -  Get one R stat per test (largest j>j') question>