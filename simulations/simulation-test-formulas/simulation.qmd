...
### I have gotten away with this before
# samples_per_experiment = 250
...
function generate_sample(mu, sd, N)
    # a function to exp every number of a vector
...
# println(length(sample1["bootstrap_means"]))
```
...
all_r_N = extract_all_rstats(sim_N[2])

println(length(all_r_N))
...
all_r_K = extract_all_rstats(sim_K[2])

println(length(all_r_K))
...
all_r_M = extract_all_rstats(sim_M[2])

println(length(all_r_M))
...
# TODO: 
# - Switch to a normal distribution
# - Fewer samples of longer vectors (max 17)
# - Switch K and M, make M "B"
# - Interrogate distributional properties of R stat
